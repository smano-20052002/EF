...
//  handleEditMaterial(EditContentUrl)
  ...
  const handleMaterial = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      console.log("po",file)
      setselectedContent(URL.createObjectURL(file));
    }
  };
                ...
                <Form.Group>
                  ...
                  <Card {...getRootProps()} className="dropzone">
                    <Card.Body className="text-center">
                      ...
                      {selectedContent ? (
                        <Card>
                          <CloseButton className='position-absolute top-0 end-0' style={{ color: 'red' }} onClick={removecontent} aria-label="remove file" />
